...
def non_max_suppression_obb(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        N = 4
        coef_sin = torch.tensor(tuple(np.sin(2 * k * np.pi / N) for k in range(N))).to(x)
        coef_cos = torch.tensor(tuple(np.cos(2 * k * np.pi / N) for k in range(N))).to(x)

        angle_cls = x[:, class_index:class_index + N] * 2 - 1
        phase_sin = torch.sum(angle_cls[:, 0:N] * coef_sin, dim=-1)
        phase_cos = torch.sum(angle_cls[:, 0:N] * coef_cos, dim=-1)
        phase_mod = phase_cos ** 2 + phase_sin ** 2

        angle_cls2 = x[:, class_index + N:class_index + 2 * N] * 2 - 1
        phase_sin2 = torch.sum(angle_cls2[:, 0:N] * coef_sin, dim=-1)
        phase_cos2 = torch.sum(angle_cls2[:, 0:N] * coef_cos, dim=-1)
        phase_mod2 = phase_cos2 ** 2 + phase_sin2 ** 2

        # Conf based on modulation, conf = obj_conf * cls_conf * ang_conf
        x[:, 5:class_index] *= torch.log(torch.clip(phase_mod2.view(-1, 1), 0.001, 1) + 20) / np.log(0.05 + 20)
        x[:, 5:class_index] *= 2

        phase = -torch.atan2(phase_sin, phase_cos)
        phase2 = -torch.atan2(phase_sin2, phase_cos2) / 2

        # Phase unwarpping, dual freq mixing
        phase3 = phase2 + torch.pi
        dif2 = torch.abs((phase2 - phase))
        dif3 = torch.abs((phase3 - phase))
        dif2[dif2 > torch.pi] = torch.pi * 2 - dif2[dif2 > torch.pi]
        dif3[dif3 > torch.pi] = torch.pi * 2 - dif3[dif3 > torch.pi]
        phase2[dif2 > dif3] = phase3[dif2 > dif3]

        # Set angle of isotropic objects to zero
        phase2[phase_mod2 < 0.47] *= 0  # Threshold for isotropic objects, Thr_iso

        # Phase to angle
        theta_pred = phase2.view(-1, 1) / 2
