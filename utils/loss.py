    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                phase_targets = tgaussian_theta[i].view(-1, 1) * 2
                phase_cos_targets = tuple(torch.cos(phase_targets + x) for x in (0, torch.pi * 0.5, torch.pi * 1.0, torch.pi * 1.5))
                phase_shift_targets = torch.cat(phase_cos_targets, axis=-1)

                # Dual-freq PS head
                phase_targets2 = tgaussian_theta[i].view(-1, 1)
                phase_targets2[phase_targets2 > torch.pi / 4] -= torch.pi / 2
                phase_targets2[phase_targets2 < -torch.pi / 4] += torch.pi / 2
                phase_targets2 *= 4
                phase_cos_targets2 = tuple(torch.cos(phase_targets2 + x) for x in (0, torch.pi * 0.5, torch.pi * 1.0, torch.pi * 1.5))
                phase_shift_targets2 = torch.cat(phase_cos_targets2, axis=-1)

                phase_shift_dual_targets = torch.cat((phase_shift_targets, phase_shift_targets2), axis=-1)

                # ltheta += self.BCEtheta(ps[:, class_index:class_index + 4].sigmoid() * 2 - 1, phase_shift_targets)
                ltheta += self.BCEtheta(ps[:, class_index:class_index + 8].sigmoid() * 2 - 1, phase_shift_dual_targets)
